Function Get-ExchangeCachedMode {
    ...
    Begin {
        ...
        $scriptblock = {
            ...
            $SIDs = $rootkey.GetSubKeyNames() | Where-Object {
            ...
            $Hash = @{}
            $SIDs | ForEach-Object {
                $Hash.$_ = (ConvertSID-ToUserName $_ | Select-Object -Expand Name)
                $Hash[$hash.$_] = $_
            }
            ...
            $SIDs | ForEach-Object {
                ...
                Try {
                    ...
                    If ($Key.GetSubKeyNames() -Contains "Windows Messaging Subsystem") {
                        ...
                        $Profiles.GetSubKeyNames() | ForEach-Object {
                            $OutlookProfile = $_
                            $OutlookProfileSubKey = $Profiles.OpenSubKey($OutlookProfile)
                            $OutlookProfileSubKey.GetSubKeyNames() | ForEach-Object {
                                If ($OutlookProfileSubKey.OpenSubKey("$_").GetValueNames() -Contains "00036601") {
                                    Write-Verbose ("Checking value for Cached Mode on profile: {0}" -f $OutlookProfile)
                                    $RegKey = $_
                                    $Key = $OutlookProfileSubKey.OpenSubKey($RegKey)
